...
uint8_t StrobeWidthPage::OnIncrement(uint8_t id, int8_t value) {
  ...
  SetStrobeWidth(Clamp(static_cast<int16_t>(Sync::GetStrobeWidth()) + value, kMinStrobeWidth, kMaxStrobeWidth));
...
void StrobeWidthPage::UpdateScreen() {
  ...
  DrawSelStr4(&line2[1], Sync::GetStrobeWidth(), PSTR("1ms 2ms 3ms 4ms 5ms "));
...
/* static */
void StrobeWidthPage::SetStrobeWidth(uint8_t width) {
  Sync::SetStrobeWidth(width);
  eemem.set_strobe_width(width);
}
